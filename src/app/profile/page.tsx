...
function ProfilePage() {
  ...
  useEffect(() => {
    async function fetchAppliedColleges() {
      if (user?.id || user?.email) {
        try {
          setLoadingApplications(true)
          const applications = await getUserApplications(user.id || user.email)

          const collegesWithApplications = await Promise.all(
            applications.map(async (app) => {
              const college = await getCollegeById(app.collegeId)
              return { ...app, collegeDetails: college }
            }),
          )
          setAppliedColleges(collegesWithApplications)
        } catch (error) {
  ...
  const profileStats = [
    { label: "Applications", value: appliedColleges.length.toString(), icon: BookOpen },
      ...
      <div className="container mx-auto px-4 py-8 lg:py-12">
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
          ...
          <div className="space-y-6">
            ...
            {/* Applied Colleges Card */}
            <Card className="border-0 shadow-xl bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm">
              <CardHeader>
                <CardTitle className="text-lg font-semibold">My Applied Colleges</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                {loadingApplications ? (
                  <div className="text-center py-4">
                    <Loader2 className="h-6 w-6 animate-spin mx-auto text-blue-600" />
                    <p className="text-gray-600 text-sm mt-2">Loading applications...</p>
                  </div>
                ) : appliedColleges.length > 0 ? (
                  appliedColleges.map((app) => (
                    <Link href={`/colleges/${app.collegeId}`} key={app._id}>
                      <div className="flex items-center gap-4 p-3 rounded-lg bg-gray-50 dark:bg-gray-700/50 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors cursor-pointer">
                        <div className="relative w-16 h-16 rounded-md overflow-hidden flex-shrink-0">
                          <Image
                            src={app.collegeDetails?.image || "/placeholder.svg?height=64&width=64"}
                            alt={app.collegeName}
                            width={64}
                            height={64}
                        ...
                        <div className="flex-1">
                          ...
                          <Badge variant={app.status === "accepted" ? "default" : "secondary"} className="mt-1 text-xs">
                            {app.status}
                          </Badge>
                        </div>
                      </div>
